...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        $api_sess_variable = $this->config->item('rest_sess_name');
          
		 // echo $this->input->server($api_sess_variable);
		 
        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
      $sess_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_sess_variable));
        ...
        {
			$udd = isset($this->_args[$api_sess_variable]) ? $this->_args[$api_sess_variable] : $this->input->server($sess_name);
        //echo $udd;exit;     
	
            if (!($row=$this->rest->db->get_where($this->config->item('rest_keys_table'),
            array($this->config->item('rest_key_column')=>$key,
               $this->config->item('rest_sess_column')=>$udd))->row()))
             {
                return FALSE;
